...
#define REWARD_WIN    4.0f
#define REWARD_LOSS  -4.0f
#define REWARD_INTER  1.7f
...
void ArmPlugin::onCollisionMsg(ConstContactsPtr &contacts)
{
	...
	for (unsigned int i = 0; i < contacts->contact_size(); ++i)
	{
		...
		if (collisionCheck)
		{
			// PARTEE 2 DEL GRIPPER
			//Para activar la mano del robot
			// si el griper esta tocando
			if (strcmp(contacts->contact(i).collision2().c_str(), COLLISION_POINT) == 0){
				rewardHistory = REWARD_WIN*2;
			}
      else{
             rewardHistory = REWARD_LOSS*0.5f;
      }
		...
		*/
		...
		if(!checkGroundContact)
		{
			...
			if( episodeFrames > 1 )
			{
				...
				// Linear reward
				rewardHistory = REWARD_INTER*avgGoalDelta - 0.14f;
